...
// Gabarit d'un article approfondi du guide PicRights AFP.
export default function AfpArticleLayout({ slug, lead, blocks, faq }) {
  ...
  return (
    <div style={s.page}>
      <BreadcrumbJsonLd items={[
        ...
        { name: 'Guide PicRights AFP', url: `${BASE}/picrights-afp/` },
        { name: meta?.crumb || meta?.title, url: `${BASE}/picrights-afp/${slug}/` },
      ]} />
      <JsonLd data={{
        ...
        headline: meta?.title,
        description: lead,
        author: { '@type': 'Person', name: 'Alexandre Lazarègue', jobTitle: 'Avocat au Barreau de Paris', url: `${BASE}/a-propos/` },
        publisher: { '@type': 'Organization', name: 'Lazarègue Avocats', url: BASE },
        datePublished: '2026-07-07',
        dateModified: '2026-07-07',
        mainEntityOfPage: { '@type': 'WebPage', '@id': `${BASE}/picrights-afp/${slug}/` },
      }} />
      ...
      <div style={{ padding: '32px 0 0' }}>
        <nav style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 20 }}>
          ...
          <Link href="/picrights-afp/" style={{ color: 'var(--muted)', textDecoration: 'none' }}>Guide PicRights AFP</Link>{' → '}{meta?.crumb}
        </nav>
        <div style={{ display: 'inline-block', fontSize: 11, color: 'var(--blue-mid)', background: 'var(--blue-light)', padding: '3px 10px', borderRadius: 20, marginBottom: 12 }}>Article du guide PicRights AFP</div>
      ...
      <div style={{ background: 'var(--navy)', borderRadius: 12, padding: '24px 26px', margin: '0 0 28px' }}>
        <div style={{ fontSize: 15, fontWeight: 500, color: 'white', marginBottom: 8, lineHeight: 1.5 }}>Vous souhaitez comprendre l’ensemble d’une réclamation PicRights AFP ?</div>
        <Link href="/picrights-afp/" style={{ color: '#7ec8e3', fontSize: 14, fontWeight: 600, textDecoration: 'none' }}>→ Consulter le guide complet PicRights AFP</Link>
      </div>

      <CtaBanner title="Faire examiner votre réclamation PicRights AFP" subtitle="Chaque dossier examiné personnellement par un avocat — forfait 200 € HT, prise en charge sous 48h." />
      ...
      <div style={{ padding: '12px 0 28px', borderTop: '0.5px solid var(--border)' }}>
        <div style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--muted)', margin: '14px 0 10px' }}>Tous les articles du guide PicRights AFP</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          <Link href="/picrights-afp/" style={{ fontSize: 12, fontWeight: 500, color: 'var(--blue-mid)', background: 'var(--blue-light)', borderRadius: 8, padding: '6px 12px', textDecoration: 'none' }}>Guide complet PicRights AFP</Link>
          {rest.map((a) => (
